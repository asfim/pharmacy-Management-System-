...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-slate-500">Manage all product categories here.</p>
        </div>
        <a href="{{ route('admin.categories.create') }}" class="bg-teal-600 hover:bg-teal-700 text-white font-medium py-2 px-4 rounded-lg flex items-center transition shadow-sm">
                ...
                <tbody class="divide-y divide-slate-200 text-sm">
                    @forelse($categories as $category)
                        <tr class="hover:bg-slate-50 transition">
                            <td class="px-6 py-4">
                                @if($category->image)
                                    <img src="{{ asset('storage/'.$category->image) }}" alt="{{ $category->name }}" class="w-10 h-10 object-cover rounded-lg border border-slate-200">
                                @else
                            ...
                            <td class="px-6 py-4 flex items-center justify-end space-x-3">
                                <a href="{{ route('admin.categories.edit', $category) }}" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-2 rounded-lg transition" title="Edit">
