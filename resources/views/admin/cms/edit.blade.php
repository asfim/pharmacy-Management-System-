...
<div class="flex justify-between items-center mb-6">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Edit Feature</h2>
        <p class="text-sm text-slate-500 mt-1">Update the details of this homepage feature box.</p>
    </div>
...
<div class="bg-white rounded-2xl border-2 border-slate-200 shadow-md hover:shadow-xl transition-shadow max-w-3xl p-8">
    <form action="{{ route('admin.cms.update', $cm->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    ...
                    <select name="section" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50" required>
                        <option value="feature_strip" {{ old('section', $cm->section) == 'feature_strip' ? 'selected' : '' }}>Feature Strip (4 Boxes below Hero)</option>
                        <option value="why_choose_us" {{ old('section', $cm->section) == 'why_choose_us' ? 'selected' : '' }}>Why Choose Us (List with icons)</option>
                        <option value="why_choose_stats" {{ old('section', $cm->section) == 'why_choose_stats' ? 'selected' : '' }}>Why Choose Us Stats (Right side 4 boxes)</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Title <span class="text-rose-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title', $cm->title) }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50" required placeholder="e.g. Genuine Medicines">
                ...
                <div>
                    ...
                    <select name="color" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50" required>
                        <option value="emerald" {{ old('color', $cm->color) == 'emerald' ? 'selected' : '' }}>Emerald (Green)</option>
                        <option value="blue" {{ old('color', $cm->color) == 'blue' ? 'selected' : '' }}>Blue</option>
                        <option value="violet" {{ old('color', $cm->color) == 'violet' ? 'selected' : '' }}>Violet (Purple)</option>
                        <option value="amber" {{ old('color', $cm->color) == 'amber' ? 'selected' : '' }}>Amber (Yellow)</option>
                        <option value="rose" {{ old('color', $cm->color) == 'rose' ? 'selected' : '' }}>Rose (Red)</option>
                        <option value="teal" {{ old('color', $cm->color) == 'teal' ? 'selected' : '' }}>Teal</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="description" value="{{ old('description', $cm->description) }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50" placeholder="e.g. 100% authentic products">
            </div>
            
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Icon (SVG Code)</label>
                <textarea name="icon" rows="3" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50 font-mono text-xs" placeholder="<svg>...</svg>">{{ old('icon', $cm->icon) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50" required>
                        <option value="active" {{ old('status', $cm->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $cm->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Display Order <span class="text-rose-500">*</span></label>
                    <input type="number" name="order" value="{{ old('order', $cm->order) }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 transition-all bg-slate-50" required>
                </div>
            </div>
        </div>
