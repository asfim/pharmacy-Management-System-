...
@php
    $header = 'Add New subSub Category';
@endphp

@section('content')
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Add subSub Category</h2>
            <p class="text-sm text-slate-500">Create a new product subSub Category</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-3xl">
        <form action="{{ route('admin.sub-categories.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="parent_subSub Category_id" id="parent_subSub Category_id" 
                    ...
                    @foreach($categories as $parent)
                        <option value="{{ $parent->id }}" {{ old('parent_subSub Category_id') == $parent->id ? 'selected' : '' }}>
                            {{ $parent->name }}
                        </option>
                    @endforeach
                </select>
